...
import { saveCustomTable, type CustomTable } from '../tables';
import { useApp } from '../store';

// Import a photo, position it inside a 16:9 frame (the TV's shape), and what
// shows through the frame becomes the table surface. Saved rasterized to
// localStorage on this laptop and broadcast to phones when selected — the
// exact custom-back flow, at table scale.

const FRAME_W = 480; // on-screen frame; 16:9 exactly
const FRAME_H = 270;
const OUT_W = 1600; // saved raster; big enough for a TV, cheap enough for LAN
const OUT_H = 900;
// theme:set rejects payloads over 1.5M chars; leave headroom for the envelope.
const MAX_DATA_URL_CHARS = 1_400_000;

interface Props {
  onClose: () => void;
  onSaved: (table: CustomTable) => void;
}

export function TableMaker({ onClose, onSaved }: Props) {
  ...
  const save = () => {
    ...
    // Step quality down until the data URL fits the broadcast cap.
    let dataUrl = '';
    ...
    if (dataUrl.length > MAX_DATA_URL_CHARS) {
      pushToast('That image is too detailed to compress — try a simpler one.');
      return;
    }
    try {
      const table = saveCustomTable(name || 'Custom table', dataUrl, themeId ?? 'royal-sapphire');
      onSaved(table);
    } catch {
      pushToast('Could not save the table — local storage may be full.');
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-6">
      ...
      >
        ...
        <p className="mb-5 text-center font-ui text-sm text-ivory/70">
          Drag to position · scroll or slide to zoom · what's in the frame is your table
        </p>
